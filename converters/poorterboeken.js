...
var fileNameOut = 'poorterboeken.graphson.json';
var source = 'poorterboeken';

var grex = require('grex'),
    argv = require('optimist')
      .usage('Transforms Poorterboeken data set into GraphSON format.\nUsage: $0')
      .demand('f')
      .alias('f', 'file')
  ...
  parse(fs.readFileSync(argv.file, {encoding: 'utf8'}), {delimiter: ','}, function(err, data) {
    ...
    for (var i=0; i<data.length; i++) {
      ...
      if ((!containsObject(uri, usedURIs)) &&  (obj[2] != "") && (!containsObject(checkObj, currentPlacesList))) {
        ...
        var geometry = ((obj[5] != 0) && (obj[6] != 0)) ? {"type": "Point", "coordinates": [parseFloat(obj[6]), parseFloat(obj[5])]} : "";
        ...
        currentPlacesURIs[JSON.stringify({name: obj[2], tgn: obj[10], gn: obj[9]})] = uri;
      }
    }
    ...
    for (var i=0; i<data.length; i++) {
      ...
      var key = JSON.stringify({name: obj[2], tgn: obj[10], gn: obj[9]});
